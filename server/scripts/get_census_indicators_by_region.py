...
MONTREAL_CD = "2466"
LAVAL_CD = "2465"  # 2465005 SDR
TERREBONNE_CD = "2464"  # 2464008 SDR - Les moulins = 2464
LEVIS_CD = "2425"  # 2425213 SDR
REPENTIGNY_CD = "2460"  # 2460013 SDR - L'Assomption MRC = 2460
LONGUEUIL_CD = "2462"  # 2458227 SDR - Longueuil = 2462

TARGET_CD = TERREBONNE_CD
TARGET_NAME = "Terrebonne"

CACHE_PATH = Path(f"/tmp/indicateurs_{TARGET_CD}.csv")
...
INDICATEURS = {
    ...
    1451: "logement_reparations_majeures",  # Réparations majeures requises
    1467: "logement_30pct_revenu",  # 30 % ou plus du revenu consacré logement
    1472: "logement_taille_non_convenable",  # Logement de taille non convenable
    2230: "taux_chomage",  # Taux de chômage (%)
}

# 1. Charger et filtrer le shapefile pour la région cible
print("Chargement du shapefile...")
gdf_qc = gpd.read_file(DATASET_DIR / "lad_000b21a_f" / "lad_000b21a_f.shp")
gdf = gdf_qc[gdf_qc["ADIDU"].str.startswith(TARGET_CD)].copy()
print(f"  → {len(gdf)} aires de diffusion de {TARGET_NAME} chargées")
...
if not CACHE_PATH.exists():
    ...
    p1 = subprocess.Popen(
        ["grep", "-aP", f'"{TARGET_CD}[0-9]{{4}}","Aire de diffusion"', str(CSV_PATH)],
...
df = pd.read_csv(
    CACHE_PATH, encoding="latin-1", names=col_names, header=None, low_memory=False
)
...
col_id = next(
    c
    for c in df.columns
    if "CARACT" in c.upper() and "ID" in c.upper() and "NOM" not in c.upper()
)
...
pivot = df[df[col_id].isin(INDICATEURS.keys())][
...
print(
    gdf_result[cols_display].dropna(subset=["age_moyen"]).head(5).to_string(index=False)
)

print(f"\nStatistiques {TARGET_NAME} :")
labels = [
    ...
    (
        ...
        "Logements nécessitant réparations majeures (nb)",
    ),
...
for col, label in labels:
    if col in gdf_result.columns:
        ...
        print(
            f"  {label:<50} : {val:.1f}" if not pd.isna(val) else f"  {label:<50} : N/A"
        )

# 5. Export CSV
OUTPUT_CSV = (
    Path(__file__).parent.parent / "dataset" / f"indicateurs_census_{TARGET_CD}.csv"
)
cols_export = ["ADIDU"] + [
    c
    for c in [
        ...
        "gini",
        "taux_chomage",
        "logement_30pct_revenu",
        "logement_reparations_majeures",
        "logement_taille_non_convenable",
    ]
