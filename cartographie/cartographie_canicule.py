...
if __name__ == "__main__":
    ...
    systeme.ajouter_vulnerabilite_sociale(
        ...
        poids=0.50,  # 50 % du score total
    )

    # ── COUCHES TERRITORIALES ────────────────────────────────────────────

    # 1. ÎLOTS DE CHALEUR (température de surface par satellite 2023)
    #    5 polygones couvrant Montréal — overlay rapide
    systeme.ajouter_couche_risque(
        nom='ilots_chaleur',
        chemin=str(base_path / 'vuln_territoriales/ilots_chaleurs/ilots-de-chaleur-images-satellite-2023/ilots-de-chaleur-images-satellite-2023.shp'),
        colonne_valeur='Temp_Class',  # Classe de température (1-5)
        poids=0.30,                   # 30 % du score total
        use_spatial_index=False,
    )

    # 2. CANOPÉE 2019 (couche PROTECTRICE — la végétation réduit la chaleur)
    #    Absence de canopée → risque élevé · ~514 000 polygones → R-Tree
    systeme.ajouter_couche_risque(
        nom='canope',
        chemin=str(base_path / 'vuln_territoriales/canope-2019/canopee-2019.shp'),
        colonne_valeur=None,   # présence/absence
        poids=0.20,            # 20 % du score total
    ...
    systeme.visualiser("carte_canicule.html")
    systeme.exporter_resultats("zones_risque_canicule.geojson")

    print("\n✅ Analyse terminée avec succès !")
    print("📂 Fichiers générés :")
    print("   • carte_canicule.html")
    print("   • zones_risque_canicule.geojson  (tous indicateurs inclus)")

    webbrowser.open("carte_canicule.html")
